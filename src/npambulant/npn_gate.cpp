...
#ifdef	XP_MACOSX
NPBool NPN_ConvertPoint(NPP npp, double sourceX, double sourceY, NPCoordinateSpace source, double* destX, double* destY, NPCoordinateSpace dest)
{
	if (NPNFuncs.convertpoint != NULL) {
		return NPNFuncs.convertpoint(npp, sourceX, sourceY, source, destX, destY, dest);
	}
	return false;
}
#endif//XP_MACOSX
