...
static common::renderer_private_id my_renderer_id = (common::renderer_private_id)"gtk_html_renderer";

class gui::gtk::browser_container : public ref_counted_obj {
    KHTMLPart *m_browser;
    int m_generation;
...
gtk_html_renderer::gtk_html_renderer(
    ...
    m_html_browser(NULL) 
{
	...
	AM_DBG lib::logger::get_logger()->debug("gtk_html_renderer(0x%x)",this);
}

void 
gui::gtk::gtk_html_renderer::start(double t) {
	m_lock.enter();
 	AM_DBG lib::logger::get_logger()->debug("gtk_html_renderer::start(0x%x)", this);
	...
	if (m_html_browser == NULL) {
		...
		gtk_ambulant_widget* qaw = aqw->get_ambulant_widget();
		assert(qaw);
	...
	AM_DBG lib::logger::get_logger()->debug("gtk_html_renderer::start(0x%x) html_widget=0x%x",this,browser);
...
void
gui::gtk::gtk_html_renderer::stop() {
	m_lock.enter();
	AM_DBG lib::logger::get_logger()->debug("gtk_html_renderer::stop(0x%x)", this);
