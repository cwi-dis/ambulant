...
; Finish page
!define MUI_FINISHPAGE_RUN "$INSTDIR\AmbulantPlayerDX.exe"
...
OutFile "Ambulant-${PRODUCT_VERSION}-win32xp.exe"
...
!ifdef DISTRIBUTE_DLL_BUILD
  File /ONAME=AmbulantPlayerDX.exe "..\..\bin\win32\AmbulantPlayerDX.exe"
  File "..\..\bin\win32\libambulantDX_shwin32.dll"
  File "..\..\bin\win32\libamplugin_state_xpathDX.dll"
  File "..\..\bin\win32\libamplugin_ffmpegDX.dll"
  File "..\..\bin\win32\libamplugin_xercesDX.dll"
  File "..\..\bin\win32\avcodec-53.dll"
  File "..\..\bin\win32\avformat-53.dll"
...
!endif
  ...
  CreateShortCut "$SMPROGRAMS\Ambulant\${PRODUCT_NAME} ${PRODUCT_VERSION_BASE}\Ambulant Player.lnk" "$INSTDIR\AmbulantPlayerDX.exe"
  CreateShortCut "$DESKTOP\Ambulant Player.lnk" "$INSTDIR\AmbulantPlayerDX.exe"
...
Section "Python Plugin" PythonSection
  SetOutPath "$INSTDIR"
  File "..\..\bin\win32\libamplugin_pythonDX.dll"
...
Section -Post
  ...
  WriteRegStr HKCR "ambulant\shell\open\command" "" "$INSTDIR\AmbulantPlayerDX.exe $\"%1$\""

  WriteRegStr HKLM "${PRODUCT_DIR_REGKEY}" "" "$INSTDIR\AmbulantPlayer.exe"
  WriteRegStr ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_UNINST_KEY}" "DisplayName" "$(^Name)"
  WriteRegStr ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_UNINST_KEY}" "UninstallString" "$INSTDIR\uninst.exe"
  WriteRegStr ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\AmbulantPlayerDX.exe"
...
Section Uninstall
  ...
  Delete "$INSTDIR\libambulantDX_shwin32.dll"
  Delete "$INSTDIR\libamplugin_state_xpathDX.dll"
  Delete "$INSTDIR\libamplugin_ffmpegDX.dll"
  Delete "$INSTDIR\libamplugin_xercesDX.dll"
  ...
  Delete "$INSTDIR\libamplugin_pythonDX.dll"
