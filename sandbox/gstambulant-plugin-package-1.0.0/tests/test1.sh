#!/bin/sh
# set -x
if  xvinfo|grep "no adaptors"  ; then echo "\n*** No XVideo available, maybe nothing to see. ***\n"; fi
...
if  [ $? ]
then   gst-launch-1.0 --gst-debug-level=2 ambulantsrc ! video/x-raw,format=RGB,width=300,height=260,bpp=24,depth=24,framerate=30/1,endianness=4321,pixel-aspect-ratio=5/4,red_mask=16711680,green_mask=65280,blue_mask=255 ! videoscale ! videoconvert ! ximagesink < $srcdir/input
fi
